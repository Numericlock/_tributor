...
@section('content')
		<div class="content">
			...
			@if($parent_post->attached_count > 0)
				<div class="users-content" id="{{ 'post_'.$parent_post->posts_id }}" style="background-image:url(/img/post_img/{{$parent_post->posts_id.'_0.png'}});">

			@else
				<div class="users-content" id="{{ 'post_'.$parent_post->posts_id }}">

			@endif
				<div class="parent-users-information-wrapper">
					<div class="users-icon users-content-modal-open">
						<img src="/img/icon_img/{{$parent_post->users_id}}.png" onclick="users_href(this)" onmouseenter="users_content_modal_open(this); users_content_modal_close_reset()" onmouseleave="users_content_modal_close(this)" data-modalid="{{ $parent_post->users_id }}">
					</div>
					<div class="users-information content-modal-open">
						<span class="users-information-name" onmouseenter="users_content_modal_open(this); users_content_modal_close_reset()" onmouseleave="users_content_modal_close(this)" data-modalid="{{ $parent_post->users_id }}">{{ $parent_post->users_name }}</span>
						<span class="users-information-id" onmouseenter="users_content_modal_open(this); users_content_modal_close_reset()" onmouseleave="users_content_modal_close(this)" data-modalid="{{ $parent_post->users_id }}">{{ "@".$parent_post->users_id }}</span>
					</div>
				</div>
				<div class="users-content-sentence">
					...
					<div class="swiper-container parent-swiper-container" data-id="{{ $parent_post->posts_id }}"  data-num="0"  data-maxnum="{{ $parent_post->attached_count }}">
						...
						<div class="swiper-wrapper">
							<!-- Slides -->
							@for ($i = 0; $parent_post->attached_count > $i ; $i++)
							<div class="swiper-slide" style="margin:0 auto;">
								<img src="/img/post_img/{{$parent_post->posts_id.'_'.$i.'.png'}}" onclick="attached_modal_open(this)" data-num="{{ $parent_post->attached_count }}" >
							</div>
							@endfor
						</div>
						...
						<div class="swiper-button-prev" onclick="swiper_prev(this);" data-id="{{ $parent_post->posts_id }}"  data-num="0"  data-maxnum="{{ $parent_post->attached_count }}"></div>
						<div class="swiper-button-next" onclick="swiper_next(this);"></div>
					</div>
					@elseif($parent_post->attached_count == 1)
